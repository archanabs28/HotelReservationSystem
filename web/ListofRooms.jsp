...
<%
   ...
   // String location=request.getParameter("location");
    String location = (String) session.getAttribute("Location");
      ...
      session.setAttribute("messageStreet", request.getParameter("messageStreet"));
                                    ...
                                    <table  style="width:80%" cellspacing="0" cellpadding="0">
                                        ...
                                        %>
                                          ...
                                          <tr>
                                              ...
                                              <td><div class="img"><img src="img/sd.jpg" width="140" height="100"> </img></div>
                                            ...
                                            <td style="color: #000" width="30%"><div>$<%=roomList.get(i).getPrice()%></div><div>per night</div><a href="CardDetailsPage.jsp?hotelID=<%=bean.getHotelID()%>&roomNo=<%=bean.getRoomNo()%>&price=<%=bean.getPrice()%>&desc=<%=bean.getDescription()%>&rtype=<%=bean.getRoomType()%>">Book Room</a> </td> 
                                           
                                             <%  session.setAttribute("roomBean",roomList.get(i));
                                                     } 
                                              else{ %>
                                             <td style="color: #000" width="30%"><div><strike>$<%=roomList.get(i).getPrice()%></strike>&nbsp;&nbsp;$<%=roomList.get(i).getDiscountedPrice() %></div><div>per night</div><a href="CardDetailsPage.jsp?hotelID=<%=bean.getHotelID()%>&roomNo=<%=bean.getRoomNo()%>&price=<%=bean.getDiscountedPrice()%>&desc=<%=bean.getDescription()%>&rtype=<%=bean.getRoomType()%>">Book Room</a></td>
                                             <% session.setAttribute("roomBean",roomList.get(i));} %>
                                          </tr> 
                                        <tr class="blank_row">
